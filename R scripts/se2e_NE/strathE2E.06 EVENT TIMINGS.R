...
Events[13, "Value"] <- 270 # Carn/scav_benthos_spawning_start_day (Pers comms WCRL spawning occurs October and November in domain).
Events[14, "Value"] <- 60 # Carn/scav_benthos_spawning_duration_(days)
...
Events[22, "Value"] <- 301 # Imigration_start_day (Pers comms from Kelly/Lynne, Tunas and Swordfish general presence November to March)
Events[23, "Value"] <- 330 # Imigration_end_day_(must_be_later_than_start_day_even_if_migration_disabled)
Events[24, "Value"] <- 1 / 14 # Propn_of_peak_popn_in_model_domain_which_remains_and_does_not_emigrate (Pers comms from Kelly/Lynne, Yellowfin Tuna remains in domain, accounts for 1/14 unique species in SAU data).
Events[25, "Value"] <- 60 # Emigration_start_day (Pers comms from Kelly/Lynne, Tunas and Swordfish general presence November to March)
Events[26, "Value"] <- 90 # Emigration_end_day_(must_be_later_than_start_day_even_if_migration_disabled)
