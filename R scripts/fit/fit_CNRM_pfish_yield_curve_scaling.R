...
# plot_ycurve_scaling <- function(ycurve_scaling, guild) {
#   par(mfrow = c(2,2))
#   plot(
#     ycurve_scaling$numax, 
#     ycurve_scaling$ddmort, 
#     type = "n", 
#     xlim = rev(range(ycurve_scaling$numax)), 
#     xlab = paste0("numax ", guild), 
#     ylab = paste0("ddmort ", guild)
#   )
#   lines(ycurve_scaling$numax, ycurve_scaling$ddmort) 
#   
#   plot(
#     ycurve_scaling$numax, 
#     ycurve_scaling$catch_current, 
#     type = "n", 
#     xlim = rev(range(ycurve_scaling$numax)), 
#     xlab = paste0("numax ", guild), 
#     ylab = paste0("catch HR 1 ", guild)
#   )
#   lines(ycurve_scaling$numax, ycurve_scaling$catch_current) 
#   
#   plot(
#     ycurve_scaling$numax, 
#     ycurve_scaling$catch_msy, 
#     type = "n", 
#     xlim = rev(range(ycurve_scaling$numax)), 
#     xlab = paste0("numax ", guild), 
#     ylab = paste0("catch msy ", guild)
#   )
#   lines(ycurve_scaling$numax, ycurve_scaling$catch_msy) 
#   
#   plot(
#     ycurve_scaling$numax, 
#     ycurve_scaling$ratio, 
#     type = "n", 
#     xlim = rev(range(ycurve_scaling$numax)), 
#     xlab = paste0("numax ", guild), 
#     ylab = "catch hr 1 / catch msy "
#   )
#   lines(ycurve_scaling$numax, ycurve_scaling$ratio)
# }
...
## 1: Linear relationship between numax and ddmort parameters based on original values
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- (baseline_ddmort_pfish / baseline_numax_pfish) * numax_pfish
# 
# ycurve_scaling <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling$scalar <- "1"
# write.csv(ycurve_scaling, "./StrathE2E/Results/linear_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling, "Plank fish")
# 
# ## 2: Scalar halved
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- (baseline_ddmort_pfish / baseline_numax_pfish) * 0.5 * numax_pfish
# 
# ycurve_scaling_half <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_half$scalar <- "1/2"
# write.csv(ycurve_scaling_half, "./StrathE2E/Results/half_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_half, "Plank fish")
# 
# ## 3: Scalar 1/3
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- (baseline_ddmort_pfish / baseline_numax_pfish) * (1/3) * numax_pfish
# 
# ycurve_scaling_third <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_third$scalar <- "1/3"
# write.csv(ycurve_scaling_third, "./StrathE2E/Results/third_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_third, "Plank fish")
# 
# ## 4: Scalar 1.5x
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- (baseline_ddmort_pfish / baseline_numax_pfish) * 1.5 * numax_pfish
# 
# ycurve_scaling_onehalf <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_onehalf$scalar <- "1.5"
# write.csv(ycurve_scaling_onehalf, "./StrathE2E/Results/onehalf_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_onehalf, "Plank fish")
# 
# ## 5: Scalar doubled
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- (baseline_ddmort_pfish / baseline_numax_pfish) * 2 * numax_pfish
# 
# ycurve_scaling_double <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_double$scalar <- "2"
# write.csv(ycurve_scaling_double, "./StrathE2E/Results/double_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_double, "Plank fish")
# 
# ## 6: Scalar 16.6%
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- (baseline_ddmort_pfish / baseline_numax_pfish) * (1/6) * numax_pfish
# 
# ycurve_scaling_sixth <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_sixth$scalar <- "1/6"
# write.csv(ycurve_scaling_sixth, "./StrathE2E/Results/sixth_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_sixth, "Plank fish")
# 
# ## 7: No ddmort change
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- rep(baseline_ddmort_pfish, length(numax_pfish))
# 
# ycurve_scaling_base_ddmort <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_base_ddmort$scalar <- "constant ddmort"
# write.csv(ycurve_scaling_base_ddmort, "./StrathE2E/Results/base_ddmort_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_base_ddmort, "Plank fish")
# 
# ## 8: constant double ddmort
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- rep(baseline_ddmort_pfish * 2, length(numax_pfish))
# 
# ycurve_scaling_double_ddmort <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_double_ddmort$scalar <- "constant 2x ddmort"
# write.csv(ycurve_scaling_double_ddmort, "./StrathE2E/Results/double_ddmort_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_double_ddmort, "Plank fish")
# 
# ## 8: increasing ddmort
# increases <- 0:9
# numax_pfish <- baseline_numax_pfish - (decreases * baseline_numax_pfish)
# ddmort_pfish <- baseline_ddmort_pfish + (increases * baseline_ddmort_pfish)
# 
# ycurve_scaling_increase_ddmort <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, setup_files = setup_files)
# ycurve_scaling_increase_ddmort$scalar <- "increasing ddmort"
# write.csv(ycurve_scaling_increase_ddmort, "./StrathE2E/Results/increase_ddmort_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_increase_ddmort, "Plank fish")
# 
# ycurve_scaling_experiments <- rbind(
#   ycurve_scaling, 
#   ycurve_scaling_half, 
#   ycurve_scaling_third, 
#   ycurve_scaling_onehalf, 
#   ycurve_scaling_double, 
#   ycurve_scaling_sixth, 
#   ycurve_scaling_base_ddmort, 
#   ycurve_scaling_double_ddmort,
#   ycurve_scaling_increase_ddmort
# )
# ycurve_scaling_experiments <- pivot_longer(
#   ycurve_scaling_experiments,
#   cols = c(ddmort, catch_msy, catch_current, biomass_current, ratio),
#   names_to = "variable",
#   values_to = "value"
# )
# ycurve_scaling_experiments$variable <- factor(
#   ycurve_scaling_experiments$variable, 
#   levels=c("ddmort", "catch_current", "catch_msy", "ratio", "biomass_current")
# )
# ycurve_scaling_experiments$scalar <- factor(
#   ycurve_scaling_experiments$scalar,
#   levels=c("1/6", "1/3", "1/2", "1", "1.5", "2", "constant ddmort", "constant 2x ddmort", "increasing ddmort")
# )
# ggplot() +
#   geom_line(data = ycurve_scaling_experiments, aes(x = numax, y = value, color = scalar)) + 
#   scale_x_reverse() +
#   facet_wrap(~variable, scales="free")
# #
# 
# # Try increasing ddmort a lot to bring catch_msy down to landings (3.635080e+00)
# increases <- 0:50
# numax_pfish <- rep(baseline_numax_pfish, length(increases))
# ddmort_pfish <- baseline_ddmort_pfish + (increases * baseline_ddmort_pfish)
# hr_scaling <- rep(baseline_hrscale_pfish * 10, length(increases))
# 
# constant_numax_hr_increasing_ddmort <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, hr_scaling, setup_files = setup_files)
# constant_numax_hr_increasing_ddmort
# write.csv(ycurve_scaling_increase_ddmort, "./StrathE2E/Results/increase_ddmort_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_increase_ddmort, "Plank fish")
# 
# 
# # Try increasing overall fishing pressure on PFish while keeping other params the same
# increases <- seq(0, 25, 0.5)
# hr_scaling <- baseline_hrscale_pfish * increases
# numax_pfish <- rep(baseline_numax_pfish, length(increases))
# ddmort_pfish <- rep(baseline_ddmort_pfish, length(increases))
# 
# constant_numax_ddmort_increasing_hr <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, hr_scaling, setup_files = setup_files)
# constant_numax_ddmort_increasing_hr
# write.csv(ycurve_scaling_increase_ddmort, "./StrathE2E/Results/increase_ddmort_ycurve_scaling.csv")
# plot_ycurve_scaling(ycurve_scaling_increase_ddmort, "Plank fish")
# 
# # Have set hrscale to *   10
# # Looking for numax value that has catch_msy around target
# # Then looking for ddmort value that has ratio around 1
# 
# # 1: looking for numax value that has catch_msy around target data (3.635080e+00)
# increases <- c(seq(-0.99, -0.11, 0.11), 0, seq(0.11, 0.99, 0.11))
# numax_pfish <- baseline_numax_pfish + (baseline_numax_pfish * increases)
# hr_scaling <- rep(baseline_hrscale_pfish * 10, length(increases))
# ddmort_pfish <- rep(baseline_ddmort_pfish, length(increases))
# 
# numax_scaling <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, hr_scaling, setup_files = setup_files)
# numax_catch_msy_near_target <- numax_scaling[1, ]$numax
# 
# # 2: looking for ddmort value that makes a ratio around 1 and catch around 4 (numax of 0.08 will give a biomass around 80 and ratio around 1)
# increases <- seq(0, 10, 0.5)
# numax_pfish <- rep(baseline_numax_pfish, length(increases))
# hr_scaling <- rep(baseline_hrscale_pfish * 10, length(increases))
# ddmort_pfish <- baseline_ddmort_pfish + (baseline_ddmort_pfish * increases)
# 
# start_time <- Sys.time()
# ddmort_scaling <- yield_curve_scaling(model, numax_pfish, ddmort_pfish, hr_scaling, setup_files = setup_files)
# end_time <- Sys.time()
# 
...
model_n$data$fleet.model$HRscale_vector[""]
