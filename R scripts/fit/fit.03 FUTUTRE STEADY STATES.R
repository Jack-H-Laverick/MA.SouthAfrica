...
pmap(runs, safely(function(Force, S, Start, Stop, Years){
        ...
        model <- e2e_read(implementation, str_glue("{as.character(Start)}-{as.character(Stop)}-{Force}-{S}"),
                          ...
                          model.ident = stringr::str_glue("{as.character(Start)}-{as.character(Stop)}-{Force}-{S}"))
        ...
        file.rename(stringr::str_glue("./StrathE2E/{implementation}/{as.character(Start)}-{as.character(Stop)}-{Force}-{S}/Param/initial_values-{as.character(Start)}-{as.character(Stop)}-{Force}-{S}.csv"),
        ...
        unlink(stringr::str_glue("./StrathE2E/{implementation}/{as.character(Start)}-{as.character(Stop)}-{Force}-{S}/Param/initial_values_{toupper(implementation)}_{as.character(Start-10)}-{as.character(Stop-10)}-{Force}-{S}.csv"))
        unlink(stringr::str_glue("./StrathE2E/{implementation}/2010-2019-{Force}-{S}/Param/initial_values_2010-2015-{Force}-{S}.csv"))
