...
Physical_parameters <- read.csv(stringr::str_glue("./StrathE2E/Models/{implementation}/2010-2019/Param/physical_parameters_CELTIC_SEA.csv")) # Read in example Physical drivers
#Physical_parameters <- read.csv(stringr::str_glue("./StrathE2E/Models/{implementation}/2010-2019/Param/physical_parameters_{toupper(implementation)}.csv")) # Read in example Physical drivers
...
Physical_parameters[1,"Value"] <- filter(My_space, Shore == "Offshore", slab_layer == "S")$Elevation * -1 # Offshore_Shallow_layer_thickness_(m)
Physical_parameters[2,"Value"] <- filter(My_space, Shore == "Offshore", slab_layer == "D")$Elevation * -1 # Offshore_Deep_layer_thickness_(m)
Physical_parameters[3,"Value"] <- filter(My_space, Shore == "Inshore", slab_layer == "S")$Elevation * -1  # Inshore_Shallow_layer_thickness_(m)
...
write.csv(Physical_parameters,
          file = stringr::str_glue("./StrathE2E/Models/{implementation}/2010-2019/Param/physical_parameters_{toupper(implementation)}.csv"), 
          row.names = F)
